...
## TAI
tai <- map_dfr(.x = 1:length(ssps), .f = function(s){
  map_dfr(.x = 1:length(gcms), .f = function(g){
    to.check(ind = 'TAI', ssp = ssps[s], gcm = gcms[g])
  })
})
tai
write.csv(tai, './tbl/index-values/tai.csv', row.names = FALSE)
    ...
    rslt <- map_dfr(.x = gcms, .f = function(g){
      ...
      rsl <- map_dfr(.x = unique(pull(dfm, iso)), .f = function(i){
        ...
        sbd <- dfm %>%  
          ...
          arrange(date) %>%  
          mutate(
            trend = zoo::rollmean(value, k = 3, fill = NA),
            trend_loess = predict(loess(value ~ as.numeric(date), span = 0.5))
          ) %>% 
          pull(trend)
        
        sbd <- tibble(iso = i, gcm = g, ssp = s, date =  seq.Date(from = as.Date("2021-01-01"), by = "year", length.out = length(sbd)), trend = sbd)
        return(sbd)
