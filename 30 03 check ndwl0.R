...
indx <- 'NDWL0'
dirs <- map(dirs, dir_ls, regexp = indx)
dirs <- unlist(dirs)
...
to.check <- function(ind, ssp, gcm){
  ...
  dir <- grep(paste0(ssp, '_', gcm), dirs, value = T)
  fls <- dir_ls(dir) %>% as.character() %>% grep(ind, ., value = T)
  fls <- grep(paste0('/', ind, '/', ind), fls, value = T)
...
ndwl0 <- map_dfr(.x = 1:length(ssps), .f = function(s){
  map_dfr(.x = 1:length(gcms), .f = function(g){
    to.check(ind = 'NDWL0', ssp = ssps[s], gcm = gcms[g])
  })
})
write.csv(ndwl0, './tbl/index-values/ndwl0.csv', row.names = FALSE)
